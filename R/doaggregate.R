...
doaggregate <- function(sites2blocks, countcols=NULL, popmeancols=NULL, calculatedcols=NULL, testing=FALSE, ...) {
  ...
  # unless need save that to analyze distance distribution 
  ...
  # Demog.Index <- weighted.mean(Demog.Index, w = pop)
  # Demog.Index <- VSI.eo
  ...
  # hard coded for now:
  varsneedpctiles <- c(ejscreen::names.e, union(ejscreen::names.d, 'pct.unemployed'), ejscreen::names.d.subgroups, ejscreen::names.ej)
  ...
  for (i in seq_along(varsneedpctiles)) {
    # using ejscreen::lookupUSA now but older code 2015 had used  EJAM::lookup.pctile.US(results_bysite[ , myvar], .....  i=1
