...
ui <- fluidPage(
  ...
  tags$head(
    ...
    # TITLE OF WEBPAGE ####
    tags$title('ejscreenapi for EJAM'),
  ...
  fluidRow(
    column(
      ...
      radioButtons('whichip',label = 'URL/IP', choices = ips, selected = whichip, inline = TRUE),
      ...
      shiny::radioButtons('usenicenames', label='Table column names', choices = list(original=FALSE,friendly=TRUE),selected = FALSE,inline = TRUE),
      shiny::downloadButton('downloadbutton', 'Download these results') #,
      # bookmarkButton(label = "Save your work to come back later")
    ),
    column(
      8,
